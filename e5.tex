...
Previous research pins this increase either on gestures helping the speaker package more complex information into appropriate units for speech, or on some of the communicative load being traded off from speech to gesture.  
Support for these two views requires investigation of the relationship between speech and gesture.


vidence that they specify referents in parallel, but a general increase in gesture duration over a trial has been found when referents are less describable.


In the present study we use naturalistically-occurring dialogues, and measure the relative durations of gestures and speech directly, in order to establish whether they co-vary (as would be predicted by the packaging account) or correlate inversely (as would be predicted by a trade-off).
