...
This might be because gestures have benefits for speech, either in aiding lexical access \citep{Chawla1994} or in helping to package more complex information into appropriate units for speech\citep{Kita2000}.
Alternatively, gesture production might increase with conceptual demand because some of the communicative load is being traded off from speech to gesture \citep{Bangerter2004, DeRuiter2006}.

The relationship between speech and gesture has tended to be measured as the number of discrete gestures per word, experimental trial or some other construct. 
However, because gestures and vocalisations can vary in durations, these metrics fail to capture the relative contributions of each modality.
Moreover, few studies have investigated the speech-gesture relationship in a dialogical setting, where two people are making explicit contributions.
The present study uses naturalistically-occurring dialogues in which participants jointly engage in producing referring expressions, and measures the relative durations of gestures and speech directly, in order to establish whether they co-vary (as would be predicted by the packaging account) or correlate inversely (as would be predicted by a trade-off).

\subsubsection{parallel}
%parallel
Although gesturing has been considered to be intended to directly communicate meaning to an addressee, recent research has suggested that it may in part be motivated by the cognitive benefits it has for the speaker.
Gesturing has been found to be advantageous for speech production and planning processes \citep{Rauscher1996, Krauss1999, Rose2001, Morsella2004, Kita2000}, spatial working memory \citep{Wesp2001, Morsella2004}, conceptual planning \citep{Melinger2007}, and even doing mental arithmetic \citep{Goldin-Meadow2001}.

There have been many accounts of the mechanism by which gesturing may benefit production of speech. 
One suggestion is that gesturing increases activation on relevant items in the mental lexicon, thus facilitating access \citep{Krauss2000}. Another \citep{Hadar1997} holds that gesturing prevents visuo-spatial imagery from decaying, providing speech production processes with better quality information. In a similar vein, the Information Packaging Hypothesis \citep{Kita2000, Kita2003} claims that gesturing helps speakers to package complex information into appropriate units for speech. 

Common to all these accounts is the claim that as conceptual load increases, speech and gesture increase in parallel.
\citeauthor{So2009}'s \citeyear{So2009} study found evidence which patterned with this claim. 
...
Importantly, they did not use gesture to compensate for underspecifications in their spoken descriptions. 


%JK LINK >>>>>>

\subsubsection{tradeoff}
%alternative = trade-off. inverse correlation.
\citeauthor{So2009}'s results contrast with that of \citet{Melinger2004}.
%JK explain melingers study
...
This is supported by studies which have found increased gesture rates when describing information which is novel (as opposed to repeated) for the speaker \citep{Hoetjes2015} and for the addressee \citep{Jacobs2007}. and when referring to targets which are further away \citep{Bangerter2004}.
The fact that gesture rate per X increases here suggests that gesture may in some 
se findings suggest that speakers do appear to 
 as the effort required to verbally encode meaning increases, so does the likelihood of a gesture occurring.
...
Furthering to this confusion, rates of discrete gestures have been calculated per minute \citep{Mol2011}; per 100 words \citep{Masson-Carro2015, Hostetter2007, Gerwing2011, Hoetjes2015}; per \term{feature description} \ci
