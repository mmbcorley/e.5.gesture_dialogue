...
\begin{figure}
  \centering
	\includegraphics[width=\linewidth]{prev_plot.pdf}
...
Instead, the use of gesture is more nuanced: Communication can be viewed as occurring within a vector space of speech and gesture, which varies according to the relative effort placed in either modality.

We propose that measuring the relative durations of speech and gesture offers a clearer window into how speakers distribute effort across the two modalities, rather than defining individual gestures against various constructs of speech.
One downside of this metric is that it fails to capture the distribution of information across each channel, capturing only the distribution of effort. 
...
%duration measures - pros and cons. where to measure from. hangs etc. when is iconic iconic? when is it beat? - also applies to other measures though
