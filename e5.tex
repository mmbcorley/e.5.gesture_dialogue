...
To confirm this, utterance duration (log transformed) and length were analysed using mixed effects linear regression.
...
Random intercepts both by-shapes and by-participant were also included, with by-participant random effects of verbal codability, trial number and their interaction.
To test whether our results pattern with previous research in showing an increase of gesturing with an increase in cognitive load, both the occurrence (gesture vs.\@ no gesture) and the duration (Z-scored) of iconic gesturing was modelled with the same fixed and random effect structure using mixed effects logistic and linear regression respectively.
...
%\paragraph{gestures and disfluencies}?? include this or not? i guess i'll have to for thesis.

%\paragraph{gesturing and definite/indefinite article descriptions}
%a/an vs the/that
...
This was then included as a predictor in the above model of the speech-gesture relationship, in place of trial number. 
...
As expected, the experimental manipulation of a shape's verbal codability influenced participants utterances, with hard-to-name shapes resulting in longer utterances, both in terms of duration (\resultsLM{1.19}{0.1}{>2}) and number of words (\resultsLM{10.6}{1.18}{>2}).
%??JK this'll change for ut length
Trial number was found to predict utterance duration, with speakers producing shorter utterances for easy-to-name shapes as the experiment progressed (\resultsLM{-0.12}{0.02}{>2}), and with this effect being greater for hard-to-name shapes (\resultsLM{-0.10}{0.03}{>2}).
%??jk results of utterance length bit>
...
Patterning with the literature, for harder-to-name shapes, speakers were more likely to produce iconic gesturing (\resultsLog{}{}{}), and took longer to gesture (\resultLM{1.27}{0.10}{>2})
As with speech, speakers produced shorter gestures for easy-to-name shapes as the experiment progressed (\resultsLM{-0.06}{0.02}{>2}), and with this effect being greater for hard-to-name shapes (\resultsLM{-0.23}{0.06}{>2}).

\subsection{The speech/gesture relationship}
Durations of speech and gesture co-varied: Accross all shapes, as the duration of speech increased, so did the duration of iconic gesturing (\resultsLM{0.70}{0.04}{>2}).
